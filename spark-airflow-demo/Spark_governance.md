...
```bash
# 1) tạo project (nếu cần)
gcloud projects create my-cdp-demo --set-as-default
# 2) bật billing nếu cần (qua Console)
# 3) bật API cần thiết
...
gcloud iam service-accounts create cdp-airflow-sa \
  --display-name="CDP Airflow SA"

# 5) cấp role (ban đầu dev/test: rộng, sau refine least-privilege)
PROJECT_ID=$(gcloud config get-value project)
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:cdp-airflow-sa@$PROJECT_ID.iam.gserviceaccount.com" \
...
# 6) tạo key json
gcloud iam service-accounts keys create ./gcp-cdp-key.json \
  --iam-account=cdp-airflow-sa@$PROJECT_ID.iam.gserviceaccount.com
```

## Tạo bucket GCS:
```bash
PROJECT=my-cdp-demo-01
GS_BUCKET=my-cdp-bronze

# tạo bucket (ví dụ regional = us-central1)
gsutil mb -p $PROJECT -l US-CENTRAL1 gs://$GS_BUCKET
# bật versioning (tùy ý)
gsutil versioning set on gs://$GS_BUCKET
```

## Tạo dataset BigQuery:

```bash
bq --project_id=$PROJECT mk --location=US my_cdp_dataset
```

## Tạo Pub/Sub topic (dùng cho realtime events):

```bash
gcloud pubsub topics create cdp-events --project=$PROJECT
```

## Tạo subscription (consumer) cho topic
### Tạo subscription pull (thường dùng cho local testing / Airflow consumer):

```bash
gcloud pubsub subscriptions create cdp-events-sub \
  --topic=cdp-events \
  --ack-deadline=60 \
  --project=my-cdp-demo-01
```
...
```bash
gcloud pubsub subscriptions list --project=my-cdp-demo-01
```


## Test publish / pull (kiểm tra ngay)
1.	Publish một thông điệp thử:
```bash
gcloud pubsub topics publish cdp-events --message='{"event":"test","ts":"'"$(date -Iseconds)"'"}' --project=my-cdp-demo-01
```
2.	Pull message (từ máy local hoặc trong container airflow nếu có gcloud và auth):

```bash
gcloud pubsub subscriptions pull cdp-events-sub --limit=5 --auto-ack --project=my-cdp-demo-01
```
## Tạo GCS bucket và BigQuery dataset (nếu chưa có)

1.	Tạo bucket (bucket name toàn cầu — đổi my-cdp-demo-bucket-123 cho unique):

```bash
gsutil mb -p my-cdp-demo-01 -c regional -l asia-southeast1 gs://my-cdp-demo-bucket-123
```
2.	Tạo dataset BigQuery:

```bash
bq --location=asia-southeast1 mk --dataset my-cdp-demo-01:cdp_raw
```

3.	(Tuỳ chọn) tạo bảng mẫu:

```bash
bq mk --table my-cdp-demo-01:cdp_raw.events schema.json
# hoặc tạo rỗng và load sau
```
## Cấp quyền (nếu dùng service account)

Bạn hiện dùng ADC của user (authorized_user) trong container — cho local dev ok. Nếu chuyển sang service account khi chạy Airflow production, cần cấp roles cho SA:
	•	roles/pubsub.viewer | roles/pubsub.subscriber (cho subscription)
	•	roles/storage.objectAdmin (hoặc storage.writer) cho GCS bucket
	•	roles/bigquery.dataEditor / roles/bigquery.dataOwner cho dataset

Ví dụ (gcloud run bằng account owner):

```bash
gcloud projects add-iam-policy-binding my-cdp-demo-01 \
  --member="serviceAccount:cdp-airflow-sa@my-cdp-demo-01.iam.gserviceaccount.com" \
...
```
Nếu SA không thể tạo key do org policy, bạn có 2 lựa chọn:
	•	dùng Workload Identity / Workload Identity Federation (kết nối GKE/GCE/Cloud Run) — phức tạp cho local.
	•	dùng ADC của user (tạm ổn cho dev) — bạn đã mount gcp-adc.json
## Trong Airflow — kiểm tra lại connection và test hook
Bạn đã thêm google_cloud_default với extra__google_cloud_platform__project — tốt. Kiểm tra từ container:

```bash
docker exec -it airflow-webserver bash -c "\
python3 - <<PY
from airflow.providers.google.cloud.hooks.gcs import GCSHook
h = GCSHook(gcp_conn_id='google_cloud_default')
print('Project:', h.project_id)
print('Buckets (sample):', [b.name for b in h.get_conn().list_buckets()][:5])
PY"
```
	•	file ADC hợp lệ (service account key JSON hoặc authorized_user JSON) được mount và GOOGLE_APPLICATION_CREDENTIALS trỏ tới đó hoặc
	•	extra__google_cloud_platform__key_path trong connection trỏ tới file trong container.

Bạn đã bỏ key_path ra; nếu mount ADC file gcp-adc.json là authorized_user thì storage.Client(project=project) hoạt động — bạn đã test thành công trước đó. Vẫn cần confirm gcloud và Python trong container cùng dùng cùng credentials.
